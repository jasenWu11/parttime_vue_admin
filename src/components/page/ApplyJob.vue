			...
			<!-- 	<el-table-column
				...
				<el-table-column
					...
					<template slot-scope="scope">
						<p>{{scope.row.professionName | formatStatus}}</p>
					</template>
				</el-table-column>
					...
					<template slot-scope="scope">
							<el-tag
								:type="scope.row.status == 2? 'danger'
									: scope.row.status == 0 ? 'primary'
									: scope.row.status == 1 ?'success' : 'info'"
								disable-transitions>{{scope.row.status | formatStatus2}}
							</el-tag>
					<!-- 	<p>{{scope.row.status | formatStatus2}}</p> -->
					</template>
				</el-table-column>
...
<script>
 /* eslint-disable */
// api引入
import { getList, deleteList} from '@/api/applyjob'

import AddOrUpdate from './applyjob-add-or-update'
export default {
  data () {
    return {
	   dataForm: {
  ...
  created () {
	...
	filters:{
		...
		formatStatus2(val){
			return val == 0 ? '待审核' : val == 1 ? '通过': val == 2 ? '谢绝' : '已取消';
		}
	},
  methods: {
