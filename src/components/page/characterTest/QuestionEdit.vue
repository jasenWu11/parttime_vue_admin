                  ...
                  { required: true, message: '请输入答案', trigger: 'blur' },
								...
								<el-row>
									...
									<el-col :span="5">
										...
										>
										<el-input type="number" v-model.trim="opt.grade" clearable placeholder="请输入分数" />
										</el-form-item>
									</el-col>
									...
									<el-col :span="2">
										<el-button style="margin-left: 10px" @click.prevent="removeDomain(index,idx)">删除</el-button>
									</el-col>
								</el-row>
              </el-form-item>
...
export default {
  ...
  data() {
	...
	watch:{
		$route(to,from){
			//因为 keep -alive ，所以监听路由变化来 初始化
			//此处 监听，整个项目只要路由变化 都会被监听到，因为这个在内存中
			//所以只有回到该标签时 才做重新赋值，去覆盖内存的旧值， 不覆盖时危险的（不同值时 danger）
			if(to.fullPath.indexOf('questionId') != -1){
